...
// The capability is what stands between a spec root and the spec file:
// '.extracted/specs/<capability>/spec.feature' is the source of truth, and
// '.extracted/changes/<id>/<delta-root>/<capability>/spec.feature' is a
// delta. The delta root is named by the schema — 'specs' under spec-driven,
// 'requirements' under workspace-intent-driven — so it is skipped by
// position rather than by name, and the two sides always agree.
function capabilityOf(featurePath) {
  const parts = featurePath.split('/').slice(0, -1);
  const changeAt = parts.indexOf('changes');
  const from = changeAt === -1 ? 2 : changeAt + 3;
...
function effectivePaths() {
  ...
  const deltaPaths = globSync('.extracted/changes/*/*/**/*.feature', {
    cwd: __dirname,
    posix: true,
