...
`npm test` extracts the active requirements, runs the focused daemon/custom-UI probes against the repository's Pew Agent implementation, and writes `reports/cucumber-report.html`. The probe page object starts Vitest as a controlled child process and waits for it to stop; no daemon or test process is left running.
