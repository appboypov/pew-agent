...
// Spec roots: specs/ (source of truth) and each active change's delta root.
// The schema names that root — `specs/` under spec-driven, `requirements/`
// under workspace-intent-driven — so it is never assumed here: every
// subdirectory of a change is walked, and the change's own artifacts are
// skipped because they sit directly in the change while a delta spec sits at
// <root>/<capability>/spec.md. changes/archive/ is excluded structurally
// (archive nests one level deeper than changes/<id>/) plus a defensive filter
// on the collected paths.
function collectSpecSources(openspecDir, basename) {
  ...
  if (fs.existsSync(changesDir)) {
    for (const change of fs.readdirSync(changesDir, { withFileTypes: true })) {
      if (!change.isDirectory() || change.name === 'archive') continue;
      const changeDir = path.join(changesDir, change.name);
      for (const deltaRoot of fs.readdirSync(changeDir, { withFileTypes: true })) {
        if (!deltaRoot.isDirectory()) continue;
        walk(openspecDir, path.join(changeDir, deltaRoot.name), basename, found);
      }
    }
  }
